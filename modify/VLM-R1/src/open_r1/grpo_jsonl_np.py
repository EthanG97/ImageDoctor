...
def score_reward(pred_scores, gt_scores, mode="mae"):
    if pred_scores is None or gt_scores is None:
        return 0.0
    if mode == "mae":
        return 1.0 - 3 * sum(abs(p - g) for p, g in zip(pred_scores, gt_scores))
    elif mode == "mse":
        return 1.0 - sum((p - g) ** 2 for p, g in zip(pred_scores, gt_scores))
    elif mode == "cosine":
        # Cosine similarity for normalized 4D vectors
        import numpy as np
        a = np.array(pred_scores)
        b = np.array(gt_scores)
...
def extract_scores(text):
    """Extract the 4 scores from the answer text"""
    try:
        sa = float(re.search(r"Semantic Alignment score: ([0-9.]+);", text).group(1))
        ae = float(re.search(r"Aesthetic score: ([0-9.]+);", text).group(1))
        pl = float(re.search(r"Plausibility score: ([0-9.]+);", text).group(1))
        ov = float(re.search(r"Overall Impression score: ([0-9.]+)", text).group(1))
        return [sa, ae, pl, ov]
    except Exception as e:
        return [0,0,0,0]  # or [0, 0, 0, 0] to penalize parsing errors

def extract_semantic_scores(text):
    try:
        sa = float(re.search(r"Semantic Alignment score: ([0-9.]+);", text).group(1))
        return [sa]
    except Exception as e:
        return [0]  # or [0, 0, 0, 0] to penalize parsing errors

def extract_aesthetic_scores(text):
    try:
        ae = float(re.search(r"Aesthetic score: ([0-9.]+);", text).group(1))
        return [ae]
    except Exception as e:
...
def extract_plausibility_scores(text):
    try:
        pl = float(re.search(r"Plausibility score: ([0-9.]+);", text).group(1))
        return [pl]
    except Exception as e:
...
def extract_overall_scores(text):
    try:
        ov = float(re.search(r"Overall Impression score: ([0-9.]+)", text).group(1))
        return [ov]
    except Exception as e:
...
def semantic_score_rewards(completions, solution, mode="mae", **kwargs):
    contents = [completion[0]["content"] for completion in completions]
    rewards = []

    for content, sol in zip(contents, solution):
        pred_block = re.search(r"<answer>(.*?)</answer>", content, re.DOTALL)
        gt_block   = re.search(r"<answer>(.*?)</answer>", sol, re.DOTALL)

        pred_scores = extract_semantic_scores(pred_block.group(1)) if pred_block else None
        gt_scores   = extract_semantic_scores(gt_block.group(1)) if gt_block else None

        reward = score_reward(pred_scores, gt_scores, mode=mode)
        rewards.append(reward)

        # Optional logging
        if os.getenv("DEBUG_MODE") == "true":
            log_path = os.getenv("LOG_PATH", "debug") + "_score.txt"
            current_time = datetime.now().strftime("%d-%H-%M-%S-%f")
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"\n----- {current_time} Reward: {reward:.4f} -----\n")
                f.write(f"Pred semantic Scores: {pred_scores}\n")
                f.write(f"GT semantic Scores: {gt_scores}\n")

    return rewards

def aesthetic_score_rewards(completions, solution, mode="mae", **kwargs):
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    ...
    for content, sol in zip(contents, solution):
        ...
        pred_scores = extract_aesthetic_scores(pred_block.group(1)) if pred_block else None
        gt_scores   = extract_aesthetic_scores(gt_block.group(1)) if gt_block else None
        reward = score_reward(pred_scores, gt_scores, mode=mode)
        rewards.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"\n----- {current_time} Reward: {reward:.4f} -----\n")
                f.write(f"Pred aesthetic Scores: {pred_scores}\n")
                f.write(f"GT aesthetic Scores: {gt_scores}\n")

    return rewards

def plausibility_score_rewards(completions, solution, mode="mae", **kwargs):
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    ...
    for content, sol in zip(contents, solution):
        ...
        pred_scores = extract_plausibility_scores(pred_block.group(1)) if pred_block else None
        gt_scores   = extract_plausibility_scores(gt_block.group(1)) if gt_block else None
        reward = score_reward(pred_scores, gt_scores, mode=mode)
        rewards.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"\n----- {current_time} Reward: {reward:.4f} -----\n")
                f.write(f"Pred plausibility Scores: {pred_scores}\n")
                f.write(f"GT plausibility Scores: {gt_scores}\n")

    return rewards

def overall_score_rewards(completions, solution, mode="mae", **kwargs):
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    ...
    for content, sol in zip(contents, solution):
        ...
        pred_scores = extract_overall_scores(pred_block.group(1)) if pred_block else None
        gt_scores   = extract_overall_scores(gt_block.group(1)) if gt_block else None
        reward = score_reward(pred_scores, gt_scores, mode=mode)
        rewards.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"\n----- {current_time} Reward: {reward:.4f} -----\n")
                f.write(f"Pred overall Scores: {pred_scores}\n")
                f.write(f"GT overall Scores: {gt_scores}\n")
                f.write(f"Raw Content: {content}\n")
                f.write(f"Raw GT: {sol}\n")

    return rewards

def extract_points(section_match):
    if section_match:
        points_str = section_match.group(1)
        return [(int(x), int(y)) for x, y in re.findall(r"<point>(\d+),(\d+)</point>", points_str)]
    return []

# Convert a list of points into a binary heatmap mask
def create_mask_from_locations(locations, size=(512, 512), radius=512/20):
    """
    Create a mask with values in {0.0, 0.33, 0.66, 1.0}
    using overlapping regular octagon patches centered at given locations.
    """
    mask = np.zeros(size, dtype=np.float32)

    # Precompute unit octagon vertices around origin (0, 0)
    def octagon_vertices(cx, cy, r):
        angle_step = math.pi / 4  # 8 sides
        return [
            (cx + r * math.cos(angle_step * i + math.pi / 8),
             cy + r * math.sin(angle_step * i + math.pi / 8))
            for i in range(8)
        ]

    for x, y in locations:
        # Create a mask with a single octagon
        single_patch = Image.new("F", size, 0.0)
        draw = ImageDraw.Draw(single_patch)
        octagon = octagon_vertices(x, y, radius)
        draw.polygon(octagon, fill=1.0)
        mask += np.array(single_patch)

    # Discretize: round overlap count to levels in [0, 3], then normalize
    mask = np.clip(mask, 0, 3)
    mask = np.round(mask) / 3.0

    return mask



# Mean squared error-based reward
def mse_reward(pred_mask, gt_map):
    return 1.0 - np.clip(np.mean((pred_mask - gt_map) ** 2) * 80, 0.0, 1.0)


def art_mse_reward(pred_mask, gt_map):
    return 1.0 - np.clip(np.mean((pred_mask - gt_map) ** 2) * 40, 0.0, 1.0)
...
def location_misalighment_heatmap_reward(completions, misalignment_pred, misalignment_np, **kwargs):
    rewards = []
    if len(misalignment_pred[0]) == 0:
        return [0.0] * len(completions)
    pred = _to_numpy(misalignment_pred[0][0])
    gt   = _to_numpy(misalignment_np[0][0][0])

    # Squeeze harmless singleton dims
    pred = np.squeeze(pred)  # e.g. (B,1,H,W)->(B,H,W)
    gt   = np.squeeze(gt)

    # If this rank has no samples, return zeros (or [] if you prefer)
    if pred.size == 0:
        return [0.0] * len(completions)

    # Ensure batch axes exist
    if pred.ndim == 2: pred = pred[None, ...]  # (H,W) -> (1,H,W)
    if gt.ndim   == 2: gt   = gt[None, ...]
    B = min(len(completions), pred.shape[0], gt.shape[0])

    for i in range(B):
        pred_i, gt_i = pred[i], gt[i]  # (H,W)
        H, W = pred_i.shape[-2], pred_i.shape[-1]

        # If you truly need 288x288, resize instead of branching:
        if (H, W) != (288, 288):
            pred_i = cv2.resize(pred_i, (288, 288), interpolation=cv2.INTER_LINEAR)
            gt_i   = cv2.resize(gt_i,   (288, 288), interpolation=cv2.INTER_LINEAR)

        mse = mse_reward(pred_i, gt_i)
        rewards.append( mse)

    # If completions > local batch, pad (optional)
...
def location_artifact_heatmap_reward(completions, artifact_pred, artifact_np, **kwargs):
    rewards = []
    if len(artifact_pred[0]) == 0:
        return [0.0] * len(completions)
    pred = _to_numpy(artifact_pred[0][0])
    gt   = _to_numpy(artifact_np[0][0][0])
    ...
    for i in range(B):
        ...
        mse = art_mse_reward(pred_i, gt_i)
        rewards.append(mse)
...
def extract_bboxes(text, orig_size=512, target_size=288):
...
def iou_reward_misalignment(completions, misalignment_np, **kwargs):
    rewards = []
    for comp in completions:
        pred_text = comp[0]["content"]
        misalignment_gt = _to_numpy(misalignment_np[0][0][0])
        # Extract bounding boxes

        pred_boxes = extract_bboxes(pred_text)

        if (misalignment_gt.sum() == 0) and (len(pred_boxes) == 0):
            rewards.append(1.0)
            continue
        if misalignment_gt.sum() == 0 and len(pred_boxes) > 0:
            rewards.append(0.0)
        ...
        for i in range(len(pred_boxes)):
            for j in range(i+1, len(pred_boxes)):
                ious.append(box_iou(pred_boxes[i], pred_boxes[j]))
        mean_iou = np.mean(ious) if ious else 0.0
        reward = 1.0-mean_iou  # Higher IoU means lower reward
        rewards.append(reward)

    return rewards

def iou_reward_artifact(completions, artifact_np, **kwargs):
    rewards = []
    for comp in completions:
        pred_text = comp[0]["content"]
        artifact_gt = _to_numpy(artifact_np[0][0][0])

        pred_boxes = extract_bboxes(pred_text)

        if (artifact_gt.sum() == 0) and (len(pred_boxes) == 0):
            rewards.append(1.0)
            continue
        if artifact_gt.sum() == 0 and len(pred_boxes) > 0:
            rewards.append(0.0)
...
def ave_pixel_reward_misalignment(completions, misalignment_np, **kwargs):
    rewards = []
    for comp in completions:
        pred_text = comp[0]["content"]
        misalignment_gt = _to_numpy(misalignment_np[0][0][0])
        ...
        for (x1, y1, x2, y2) in pred_boxes:
            ...
            if xi2 > xi1 and yi2 > yi1:
                region = misalignment_gt[yi1:yi2, xi1:xi2]
                per_box_means.append(region.mean())
        reward = float(np.mean(per_box_means)) if per_box_means else 0.0
        rewards.append(reward)

    return rewards


def ave_pixel_reward_artifact(completions, artifact_np, **kwargs):
    rewards = []
    for comp in completions:
        pred_text = comp[0]["content"]
        artifact_gt = _to_numpy(artifact_np[0][0][0])
        ...
        for (x1, y1, x2, y2) in pred_boxes:
            ...
            xi2, yi2 = min(W, xi2), min(H, yi2)
            if xi2 > xi1 and yi2 > yi1:
                region = artifact_gt[yi1:yi2, xi1:xi2]
                per_box_means.append(region.mean())
        reward = float(np.mean(per_box_means)) if per_box_means else 0.0
        rewards.append(reward)
...
def union_reward_artifact(completions, artifact_np, **kwargs):
    rewards = []
    for comp in completions:
        pred_text = comp[0]["content"]
        artifact_gt = _to_numpy(artifact_np[0][0][0])
        ...
        if artifact_gt.sum() > 0 and len(pred_boxes) == 0:
            ...
            continue
        H, W = artifact_gt.shape
        union_mask = np.zeros_like(artifact_gt, dtype=bool)
        for (x1, y1, x2, y2) in pred_boxes:
            xi1, yi1, xi2, yi2 = map(int, [x1, y1, x2, y2])
            # clamp
            xi1, yi1 = max(0, xi1), max(0, yi1)
            xi2, yi2 = min(W, xi2), min(H, yi2)
            if xi2 > xi1 and yi2 > yi1:
                union_mask[yi1:yi2, xi1:xi2] = True

        # --- coverage reward ---
        covered = (artifact_gt * union_mask).sum()
        total   = artifact_gt.sum()
        reward  = float(covered / (total + 1e-8))  # avoid div0
        rewards.append(reward)
    return rewards


def union_reward_misalignment(completions, misalignment_np, **kwargs):
    rewards = []
    for comp in completions:
        pred_text = comp[0]["content"]
        misalignment_gt = _to_numpy(misalignment_np[0][0][0])
        ...
        if misalignment_gt.sum() > 0 and len(pred_boxes) == 0:
            ...
            continue
        H, W = misalignment_gt.shape
        union_mask = np.zeros_like(misalignment_gt, dtype=bool)
        for (x1, y1, x2, y2) in pred_boxes:
            xi1, yi1, xi2, yi2 = map(int, [x1, y1, x2, y2])
            # clamp
            xi1, yi1 = max(0, xi1), max(0, yi1)
        ...
        # --- coverage reward ---
        covered = (misalignment_gt * union_mask).sum()
        total   = misalignment_gt.sum()
        reward  = float(covered / (total + 1e-8))  # avoid div0
        rewards.append(reward)
    return rewards
...
reward_funcs_registry = {
    ...
    "semantic_score": semantic_score_rewards,
    "aesthetic_score": aesthetic_score_rewards,
    "plausibility_score": plausibility_score_rewards,
    "overall_score": overall_score_rewards,
    "misalignment_location_heatmap": location_misalighment_heatmap_reward,
    "artifact_location_heatmap": location_artifact_heatmap_reward,
    "misalignment_boundingbox_iou": iou_reward_misalignment,
    "artifact_boundingbox_iou": iou_reward_artifact,
    "artifact_boundingbox_avepixel": ave_pixel_reward_artifact,
    "misalignment_boundingbox_avepixel": ave_pixel_reward_misalignment,
    "artifact_boundingbox_union": union_reward_artifact,
    "misalignment_boundingbox_union": union_reward_misalignment,
...
def main(script_args, training_args, model_args):
    ...
    def make_conversation_from_jsonl(example):
        if 'image_path' in example and example['image_path'] is not None:
            ...
            return {
                'image_path': [p for p in example['image_path'][0:1]],  # Store path instead of loaded image
                ...
                'prompt': [{
                    'role': 'user',
                    'content': [
                        *({'type': 'image', 'text': None} for _ in range(len(example['image_path'][0:1]))),
                        {'type': 'text', 'text': question_prompt.format(Question=example['problem'])}
                    ]
                }],
                'artifact': [p for p in example['image_path'][1:2]],
                'misalignment': [p for p in example['image_path'][2:3]], 
            }
        else:
            return {
                'problem': example['problem'],
