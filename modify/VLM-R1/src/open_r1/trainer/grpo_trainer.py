...
def plot_heatmap(gt, pred, title, vmin=0, vmax=1):
    fig, axs = plt.subplots(1, 2, figsize=(8, 4))
    axs[0].imshow(gt, cmap='hot', vmin=vmin, vmax=vmax)
    axs[0].set_title('Ground Truth')
    axs[0].axis('off')

    axs[1].imshow(pred, cmap='hot', vmin=vmin, vmax=vmax)
...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for x in inputs:
            ...
            for img in imgs:
                try:
                    ...
                    img = img.resize((512, 512), PIL.Image.Resampling.LANCZOS)
                except:
                    pass
                images.append(img)
        for x in inputs:
            if "artifact" in x and x["artifact"] is not None:
                artifact = [cv2.resize(np.load(p)/255.0, (288, 288), interpolation=cv2.INTER_LINEAR) for p in self._get_key_from_inputs(x, "artifact")]
                artifacts.append(artifact)
        
        for x in inputs:
            if "misalignment" in x and x["misalignment"] is not None:
                misalignment = [cv2.resize(np.load(p)/255.0, (288, 288), interpolation=cv2.INTER_LINEAR) for p in self._get_key_from_inputs(x, "misalignment")]
                misalignments.append(misalignment)
        inputs[0]['misalignment_np'] = np.array(misalignments)
        inputs[0]['artifact_np'] = np.array(artifacts)
        ...
        prompt_inputs = super()._prepare_inputs(prompt_inputs)[0]
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            for step_states in outputs.hidden_states[1:]:
                final_layer_output = step_states[-1] # Shape: (batch_size, 1, hidden_size)
            ...
            trimmed = [out[len(inp):] for inp, out in zip(prompt_inputs["input_ids"], generated_ids)]

            true_generated = generated_ids[:, prompt_inputs["input_ids"].shape[1]:]

            misalignment_mask = true_generated[:,1:] == unwrapped_model.config.misalignment_token_id
            artifact_mask = true_generated[:,1:] == unwrapped_model.config.artifact_token_id


            if torch.sum(misalignment_mask) == 0 or torch.sum(artifact_mask) == 0:
                zero_tensor = torch.zeros(1, 288, 288).cpu().float()
                inputs[0]['artifact_pred'] = torch.zeros(1,288,288).cpu().float()
                inputs[0]['misalignment_pred'] = torch.zeros(1,288,288).cpu().float()
                artifact_pred = zero_tensor
                misalignment_pred = zero_tensor
            else:
                image_embeds = unwrapped_model.visual(prompt_inputs["pixel_values"].to(unwrapped_model.device), grid_thw=prompt_inputs["image_grid_thw"].to(unwrapped_model.device))

                last_hidden_state = unwrapped_model.text_hidden_fcs[0](all_generated_hidden_states)
                artifact_embeddings = last_hidden_state[artifact_mask].unsqueeze(1)
                misalignment_embeddings = last_hidden_state[misalignment_mask].unsqueeze(1)

                fused_embeddings = torch.cat([misalignment_embeddings[:1,...], artifact_embeddings[:1,...]], dim=0)
                (sparse_embeddings,dense_embeddings) = unwrapped_model.prompt_encoder(points=None,boxes=None,masks=None,text_embeds=fused_embeddings)
                sparse_embeddings = sparse_embeddings.to(artifact_embeddings.dtype)
                image_embedding = unwrapped_model.image_hidden_fcs[0](image_embeds.unsqueeze(0))
                image_embedding = image_embedding.transpose(1,2).view(1, -1, 18, 18)
                low_res_masks = unwrapped_model.heatmap(
                    image_embeddings=image_embedding,
                    image_pe=unwrapped_model.prompt_encoder.get_dense_pe(),
                    sparse_prompt_embeddings=sparse_embeddings,
                    dense_prompt_embeddings=dense_embeddings,
                    multimask_output=False)
                fused_pred= unwrapped_model.sigmoid(low_res_masks)
                misalignment_pred = fused_pred[0:1,0,:,:]
                artifact_pred = fused_pred[1:, 0, :, :]
                inputs[0]['artifact_pred']     = artifact_pred.detach().cpu().to(torch.float32)     # [1,288,288]
                inputs[0]['misalignment_pred'] = misalignment_pred.detach().cpu().to(torch.float32) # [1,288,288]

            generate_returned_result = outputs.sequences
            prompt_length = prompt_ids.size(1)

            if wandb.run is not None:
                wandb.log({
                    "train/artifact_heatmap": plot_heatmap(inputs[0]['artifact_np'][0][0],artifact_pred[0].detach().cpu().to(torch.float32).numpy(), "Artifact Heatmap"),
                    "train/misalignment_heatmap": plot_heatmap(inputs[0]['misalignment_np'][0][0],misalignment_pred[0].detach().cpu().to(torch.float32).numpy(), "Misalignment Heatmap"),
                })
