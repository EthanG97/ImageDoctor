...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.output_upscaling = HeatmapPredictor(n_channels=transformer_dim)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.heatmap = MaskDecoder(transformer=TwoWayTransformer(
                depth=2,
                embedding_dim=768,
                mlp_dim=2048,
                num_heads=8,
        ...
        self.image_hidden_fcs = nn.ModuleList([nn.Sequential(*image_fc)])
        self.image_hidden_fcs.train()
        for param in self.image_hidden_fcs.parameters():
            param.requires_grad = True

        self.prompt_encoder = PromptEncoder(
            embed_dim=768,
            image_embedding_size=(18, 18),
            input_image_size=(512, 512),
            mask_in_chans=16,
        )
        self.sigmoid = nn.Sigmoid()
        self.heatmap_weight = self.config.heatmap_weight
    ...
    def forward(
        ...
        input_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        artifact: Optional[torch.Tensor] = None,
        ...
        artifact_mask = input_ids[:,1:] == self.config.artifact_token_id
        artifact_mask = torch.cat([artifact_mask, torch.zeros([artifact_mask.shape[0],1], dtype=torch.bool).to(artifact_mask.device)], dim=1)
        misalignment_mask = input_ids[:,1:] == self.config.misalignment_token_id
        misalignment_mask = torch.cat([misalignment_mask, torch.zeros([misalignment_mask.shape[0],1], dtype=torch.bool).to(misalignment_mask.device)], dim=1)

        image_mask = input_ids[:,1:] == self.config.image_token_id
        image_mask = torch.cat([image_mask, torch.zeros([image_mask.shape[0],1], dtype=torch.bool).to(image_mask.device)], dim=1)
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
        ...
        if labels is not None:
            last_hidden_state = self.text_hidden_fcs[0](hidden_states)
            ...
            fused_embeddings = torch.cat([misalignment_embeddings, artifact_embeddings], dim=0)
            (sparse_embeddings,dense_embeddings) = self.prompt_encoder(points=None,boxes=None,masks=None,text_embeds=fused_embeddings)
            sparse_embeddings = sparse_embeddings.to(artifact_embeddings.dtype)


            # image_embedding = self.image_hidden_fcs[0](hidden_states[image_mask].unsqueeze(0))
            image_embedding = self.image_hidden_fcs[0](image_embeds.unsqueeze(0))

            image_embedding = image_embedding.transpose(1,2).view(1, -1, 18, 18)
            low_res_masks = self.heatmap(
                image_embeddings=image_embedding,
                image_pe=self.prompt_encoder.get_dense_pe(),
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
                multimask_output=False)
            fused_pred= self.sigmoid(low_res_masks)
        ...
        if labels is not None:
            ...
            if artifact is not None and misalignment is not None:

                artifact_loss = ((artifact_pred - artifact) ** 2).mean()
                misalignment_loss = ((misalignment_pred - misalignment) ** 2).mean()

                if wandb.run is not None:
                    wandb.log({
                        "train/text_loss": loss.item(),
                        "train/artifact_loss": artifact_loss.item(),
                        "train/misalignment_loss": misalignment_loss.item()
                    })
                loss = loss + self.heatmap_weight * artifact_loss + self.heatmap_weight * misalignment_loss
