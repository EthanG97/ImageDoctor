...
import os
import sys
sys.path.append('/dockerx/local/flow_grpo')
import datetime
...
def compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config, mean_only=True):
    if config.train.cfg:
...
def main(_):
    ...
    while True:
        ...
        for sample in tqdm(
            ...
            sample["heatmap"] = {
                key: torch.stack(
                        [hm if isinstance(hm, torch.Tensor) else torch.as_tensor(hm)
                        for hm in value],           # stack ALL maps
                        dim=0                         # [N, H, W]
                    ).to(device=accelerator.device, dtype=torch.float32, non_blocking=True)
        ...
        samples["final_heatmap"] = samples["heatmap"]['heatmap']
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        with autocast():
                            prev_sample, log_prob, prev_sample_mean, std_dev_t = compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config, mean_only=False)
                        ...
                        log_prob_mean = log_prob.mean(dim=tuple(range(1, log_prob.ndim)))
                        sample_mean = sample["log_probs"][:,j].mean(dim=tuple(range(1, sample["log_probs"][:,j].ndim)))
                        ratio_mean = torch.exp(log_prob_mean - sample_mean)
                        si = ratio_mean.detach()
                        pitheta = torch.exp(log_prob)
                        normpitheta = pitheta.detach()
                        si = si.reshape(si.shape[0], 1, 1)
                        ratio = si * pitheta / (normpitheta + 1e-10)
                        ...
                        policy_loss = (torch.maximum(unclipped_loss, clipped_loss)).mean(dim=tuple(range(1, clipped_loss.ndim))).mean()
                        if config.train.beta > 0:
                        ...
                        info["approx_kl"].append(
                            0.5
                            * torch.mean((log_prob_mean - sample_mean) ** 2)
                        )
                        info["clipfrac"].append(
                            torch.mean(
                                (
                                    torch.abs(ratio_mean - 1.0) > config.train.clip_range
                                ).float()
                            )
                        )
                        info["clipfrac_gt_one"].append(
                            torch.mean(
                                (
                                    ratio_mean - 1.0 > config.train.clip_range
                                ).float()
                            )
                        )
                        info["clipfrac_lt_one"].append(
                            torch.mean(
                                (
                                    1.0 - ratio_mean > config.train.clip_range
                                ).float()
                            )
                        )
